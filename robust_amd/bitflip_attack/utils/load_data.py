...
# 返回val, aux, small val
def load_data_robust_amd(benign_path, malware_path, aux_num, batch_size, split_ratio):
    benign_data = np.load(benign_path)
    malware_data = np.load(malware_path)
    data_num = min(benign_data.shape[0], malware_data.shape[0])
    benign_data = torch.tensor(benign_data[:data_num, :]).to(torch.float16)
    malware_data = torch.tensor(malware_data[:data_num, :]).to(torch.float16)
    
    def get_dataloader_from_ben_mal_data(ben_data, mal_data):
        data = torch.cat((ben_data, mal_data), dim=0)
        label = torch.cat((torch.zeros(ben_data.shape[0]), torch.ones(mal_data.shape[0])))
...
# 上面函数的基础上多返回一个集合，aux_mal_num个malware，用来进行target attack。这个malware的集合的label都是0（就是目标的label，希望被分类为benign）
def load_data_robust_amd_targeted(benign_path, malware_path, aux_num, aux_mal_num, batch_size, split_ratio):
    benign_data = np.load(benign_path)
    malware_data = np.load(malware_path)
    
    aux_mal_dataset = SimpleDataset(torch.tensor(malware_data[-aux_mal_num:]).to(torch.float16), torch.zeros(aux_mal_num).to(torch.int64))
    ...
    def get_dataloader_from_ben_mal_data(ben_data, mal_data):
        data = torch.cat((ben_data, mal_data), dim=0)
        label = torch.cat((torch.zeros(ben_data.shape[0]), torch.ones(mal_data.shape[0]))).to(torch.int64)
        aux_dataset = SimpleDataset(torch_array_data=data, torch_label=label)
...
def load_data_trigger(benign_path, malware_path, aux_num, batch_size, device):
    ...
    aux_benign_dataset = SimpleDataset(benign_data[-aux_num:, :], np.zeros(aux_num).astype(np.int64), device)
    aux_malware_dataset = SimpleDataset(benign_data[-aux_num:, :], np.ones(aux_num).astype(np.int64), device)
