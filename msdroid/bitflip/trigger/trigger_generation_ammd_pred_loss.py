...
# ben_loader 和 mal_loader 每一个元素都是Data()
def update_mask_pred_loss(mal_graph_loader, gnn_model, 
                u1, u2, u3, z1, z2, z3, k, rho1, rho2, rho3, lambda_mal_loss, 
    ...
    for graph in mal_graph_loader:
        ...
        # outputs: [n,2] [[-8.1820e+00, -2.7963e-04]]
        # print(outputs, graph.y)
        # print(type(outputs), type(graph.y))
        loss = criterion(outputs, graph.y)
        assert torch.sum(torch.isnan(loss)) == 0
        malware_loss += loss
        
    malware_loss = malware_loss * lambda_mal_loss / len(mal_graph_loader)        
    # 其它loss
    ...
    total_loss.backward()
    print('malware loss:{}, loss_1:{}, loss_2:{}'.format(malware_loss, loss_1, loss_2))
    # 更新参数
...
def evaluate_mask(mal_graph_loader, gnn_model, current_mask, mask_range, device):
    mask = nn.Parameter(current_mask)
    criterion = nn.CrossEntropyLoss()
    optimizer = SGD([mask], lr=0.01)
    optimizer.zero_grad()
...
# 根据最后的pred loss来设计的攻击
# 扩展性差，没办法有比较好的攻击效果
def attack_pred_loss(mal_loader:DataLoader, gnn_model, attack_args):
    # AMDD的一些超参数
    ...
    for iter_index in range(max_iters):
        ...
        # 根据loss，更新b（trigger）
        b_new, loss = update_mask_pred_loss(mal_loader, gnn_model, 
                                  u1=u1,u2=u2,u3=u3, z1=z1, z2=z2, z3=z3, k=k_bits,
                                  rho1=rho1, rho2=rho2, rho3=rho3, lambda_mal_loss=lambda_mal_loss,
                                  current_mask=b_new, ori_mask=b_ori, mask_range=[268, 492], device=device)
        
        if True in torch.isnan(b_new):
            return -1
        ...
        if iter_index % 50 == 0:
            print('iter: %d, stop_threshold: %.6f, b_loss: %.4f' % (iter_index, max(temp1, temp2), loss))
        ...
        evaluate_mask(mal_loader, gnn_model, current_mask=real_mask, mask_range=[268, 492], device=device)
...
def attack_vec_distance(train_mal_loader:DataLoader, train_ben_loader:DataLoader, 
    ...
    for iter_index in range(max_iters):
        ...
        if (iter_index+1) % eval_iter == 0:
            ...
            real_mask[b_new > 0.5] = 1.0
            real_mask[b_new < 0.5] = 0.0
            evaluate_mask(train_mal_loader, gnn_model, current_mask=real_mask, mask_range=[268, 492], device=device)
            evaluate_mask(test_mal_loader, gnn_model, current_mask=real_mask, mask_range=[268, 492], device=device)
