...
from utils.load_data import load_data_malconv
from utils.metrics import malconv_acc
...
def main():
    ...
    val_loader, aux_loader, small_val_loader = load_data_malconv(benign_path, malware_path, 
    ...
    for ext_iter in tqdm(range(attackrargs.target_bit+10)):
        ...
        for batch_idx, (aux_inputs, _) in enumerate(aux_loader):
            ...
            clean_outputs = clean_model(aux_inputs).squeeze()
            clean_predicted = (clean_outputs > 0.5).float()

            attack_outputs = model(aux_inputs).squeeze()
