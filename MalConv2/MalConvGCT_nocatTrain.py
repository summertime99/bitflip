...
parser.add_argument('mal_train', type=file_path, help='Path to directory containing malware files for training')
parser.add_argument('ben_train', type=file_path, help='Path to directory containing benign files for training')
parser.add_argument('mal_test', type=file_path, help='Path to directory containing malware files for testing')
parser.add_argument('ben_test', type=file_path, help='Path to directory containing benign files for testing')
...
loader_threads = min(multiprocessing.cpu_count()-4, multiprocessing.cpu_count()//2+1)
#print(loader_threads)
...
base_name = "nocat_{}_batchSize_{}_lr_{}_channels_{}_filterSize_{}_stride_{}_embdSize_{}".format(
        type(model).__name__,
        BATCH_SIZE,
        lr,
        NUM_CHANNELS,
...
optimizer = optim.Adam(model.parameters(), lr=lr) # learning rate default 0.001
...
for epoch in tqdm(range(EPOCHS)):
    ...
    epoch_stats['train_auc'] = roc_auc_score(truths, preds) # 值越大越好
