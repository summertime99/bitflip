...
from utils.models import Malconv, Malconv_INT8
...
def main():
    ...
    def calculate_loss(trigger_model, detection_model, benign_loader, malware_loader, lambda_benign, lambda_malware, criterion):
        ...
        for inputs, labels in benign_loader:
            ...
            outputs = detection_model(masked_inputs).squeeze()
            loss = criterion(outputs, labels.to(torch.float32)) # loss越低，对benign分辨的准确率越高
            loss_total += lambda_benign * loss
        
        for inputs, labels in malware_loader:
            masked_inputs = trigger_model(inputs).squeeze()
            outputs = detection_model(masked_inputs)
            loss = criterion(outputs, labels.to(torch.float32)) # loss越低，对benign分辨的准确率越高，-loss越低，对malware分辨准确率越低
    ...
    while len(trigger_index) < attackrargs.flip_num:
        ...
        loss = calculate_loss(trigger_model, model, aux_benign_loader, aux_malware_loader,
        ...
        for indice in negeative_sorted_indices:
            ...
            new_loss = calculate_loss(trigger_model, model, aux_benign_loader, aux_malware_loader,
                                attackrargs.loss_benign_lambda, attackrargs.loss_malware_lambda, criterion)
